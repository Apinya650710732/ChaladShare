...
const Friends = () => {
  ...
  // โหลดข้อมูลผู้ใช้ตัวเองเพื่อเอา ownerId
  useEffect(() => {
    let cancelled = false;
  ...
  // เมื่อ ownerId มาแล้ว → โหลด incoming เสมอ และถ้าอยู่แท็บ my ก็โหลด friends
  useEffect(() => {
    if (!ownerId) return;
    fetchIncoming();
    if (activeTab === "my") fetchFriends();
  }, [ownerId, activeTab, fetchFriends, fetchIncoming]);

  // เปลี่ยนแท็บ → โหลดข้อมูลตามแท็บ
  useEffect(() => {
    if (!ownerId) return;
    if (activeTab === "my") fetchFriends();
    if (activeTab === "requests") fetchIncoming();
  }, [activeTab, ownerId, fetchFriends, fetchIncoming]);

  // เปลี่ยนหน้า (pagination) → โหลด friends ของหน้าปัจจุบัน (เฉพาะแท็บ my)
  useEffect(() => {
    if (activeTab !== "my" || !ownerId) return;
    fetchFriends(query, page);
  }, [page, activeTab, ownerId, query, fetchFriends]);

  // ค้นหาแบบ debounce (เฉพาะแท็บ my)
  useEffect(() => {
    if (activeTab !== "my" || !ownerId) return;
    const t = setTimeout(() => {
      setPage(1);
    ...
    return () => clearTimeout(t);
  }, [query, activeTab, ownerId, fetchFriends]);
          ...
          <div className="friends-topbar">
            <div className="friends-top-left">
              ...
              <div className="friends-actions">
                ...
                {/* เพิ่มปุ่ม "เพื่อนของฉัน" ให้กลับแท็บ my ได้ชัด ๆ (ถ้าไม่อยากมี ลบปุ่มนี้ได้) */}
                <button
                  type="button"
                  className={`friends-pill friends-pill--outline ${
            ...
            <>
              ...
              {!loadingReq && (
                <ul className="friends-list">
                      ...
                      <div className="friends-left">
                        ...
                        <div className="friends-name">
                          ...
                          <span className="friends-name-sub">
                            ขอเป็นเพื่อน •{" "}
                            {new Date(
                              r.requested_at || r.request_created_at || Date.now()
                            ).toLocaleDateString()}
                          </span>
                        </div>
                      </div>
                  ...
                  {incoming.length === 0 && (
                    <div className="friends-placeholder">ยังไม่มีคำขอเข้ามา</div>
                  )}
                </ul>
              )}
            </>
